...
impl CPU {
  ...
  fn execute_opcode(&mut self, opcode: u8, mmu: &mut mmu::MMU) -> i32 {
    ...
    match opcode {
      ...
      0x10 => { debug!("STOP"); println!("STOP not implemented.") },
  ...
  fn execute_cb_opcode(&mut self, opcode: u8, mmu: &mut mmu::MMU) {
    match opcode {
      ...
      0xD0 => self.explode(format!("CB: SET 2 B {:#X}", opcode)),
      0xD1 => self.explode(format!("CB: SET 2 C {:#X}", opcode)),
      0xD2 => self.explode(format!("CB: SET 2 D {:#X}", opcode)),
      0xD3 => self.explode(format!("CB: SET 2 E {:#X}", opcode)),
      0xD4 => self.explode(format!("CB: SET 2 H {:#X}", opcode)),
      0xD5 => self.explode(format!("CB: SET 2 L {:#X}", opcode)),
